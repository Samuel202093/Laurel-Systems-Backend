...
export class PlatformPayoutController {
  ...
  @Get()
  @ApiOperation({ summary: 'List payout records with filters and totals summary' })
  ...
  @ApiOperation({
    ...
    description: 'Sweeps all PENDING Korapay payouts and initiates bank transfers to your platform account.',
  })
  ...
  @Post(':id/retry')
  @ApiOperation({ summary: 'Retry a FAILED payout — re-queues it and triggers a sweep' })
  async retry(@Param('id') id: string, @Res() res: Response) {
    await this.svc.retryPayout(id);
    return res.status(HttpStatus.OK).json({ statusCode: 200, message: 'Payout re-queued and sweep triggered' });
  }
}
