...
export class AuthController {
  ...
  @ApiResponse({ status: 401, description: 'Invalid credentials' })
  async studentLogin(@Body() studentLoginDto: StudentLoginDto, @Res() res: Response) {
  ...
  @ApiOperation({
    ...
    description: 'Returns the full profile of the currently authenticated user based on their role.',
  })
  ...
  async getProfile(@Req() req: Request, @Res() res: Response) {
    ...
    const profile = await this.authService.getProfile(jwtUser.sub, jwtUser.role);
  ...
  @Post('otp/send')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Send OTP', description: 'Sends a one-time password to the given email.' })
  ...
  @Post('otp/resend')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Resend OTP', description: 'Resends the one-time password to the given email.' })
  ...
  @Post('otp/verify')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Verify OTP', description: 'Verifies the OTP code sent to the given email.' })
