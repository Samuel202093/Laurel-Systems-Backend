...
function bankTransferFee(_amountKobo: number): number { return 5_000; }

function calculateGatewayFee(amountKobo: number, processor: PaymentProcessor, channel: PaymentChannel): number {
  if (channel === PaymentChannel.BANK_TRANSFER) return bankTransferFee(amountKobo);
  switch (processor) {
    case PaymentProcessor.PAYSTACK:    return paystackFee(amountKobo);
    case PaymentProcessor.FLUTTERWAVE: return flutterwaveFee(amountKobo);
    case PaymentProcessor.KORAPAY:     return korapayFee(amountKobo);
    default:                           return paystackFee(amountKobo);
  }
}
...
function calcInstallmentKobo(fullKobo: number, total: number, n: number): number {
...
export class PaymentsService {
  ...
  async initiatePayment(dto: InitiatePaymentDto) {
    ...
    const student = await (this.prisma as any).student.findUnique({
      where: { id: studentId },
      select: { id: true, schoolId: true, email: true, firstName: true, lastName: true },
    });
    ...
    for (const item of fees) {
      ...
      if (!feeConfig) throw new NotFoundException(`Fee config ${item.feeId} not found`);

      const fullKobo = toKobo(Number(feeConfig.amount));
      const payments = await (this.prisma as any).paymentRecord.findMany({
        where: { feeId: item.feeId, studentId }, select: { amountKobo: true },
      });
      const paidKobo = payments.reduce((s: number, p: any) => s + (p.amountKobo as number), 0);

      if (paidKobo >= fullKobo) throw new BadRequestException(`"${feeConfig.feeName}" is already fully paid`);
      ...
      if (item.paymentPlan === 'installments') {
        if (!feeConfig.installments || feeConfig.installments < 2)
          throw new BadRequestException(`"${feeConfig.feeName}" does not support installments`);
        ...
        if (currentInstallment > feeConfig.installments)
          throw new BadRequestException(`All installments for "${feeConfig.feeName}" are paid`);

        amountKobo = Math.min(
          calcInstallmentKobo(fullKobo, feeConfig.installments, currentInstallment),
          fullKobo - paidKobo,
        );
      } else {
      ...
      validatedFees.push({
        feeId: item.feeId, feeName: feeConfig.feeName,
        paymentPlan: item.paymentPlan, installmentNumber, amountKobo,
      });
    }

    // 3. Calculate all charges via dynamic config
    const gatewayFeeKobo = calculateGatewayFee(principalKobo, processor, channel);
    const absorbGateway  = (school as any)?.absorbGatewayFee ?? false;

    const breakdown = await this.platformConfig.buildChargeBreakdown(
      principalKobo, gatewayFeeKobo, student.schoolId, absorbGateway,
    );
    ...
    const transaction = await (this.prisma as any).transaction.create({
      data: {
        ...
        metadata: {
          ...
          paystackSubaccountCode:  (school as any)?.paystackSubaccountCode  ?? null,
          flutterwaveSubaccountId: (school as any)?.flutterwaveSubaccountId ?? null,
        },
      },
    });
  ...
  async verifyPayment(dto: VerifyPaymentDto) {
    ...
    const transaction = await (this.prisma as any).transaction.findUnique({ where: { reference } });
    ...
    if (verification.status !== 'success') {
      await (this.prisma as any).transaction.update({ where: { id: transaction.id }, data: { status: 'FAILED' } });
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const updated = await (tx as any).transaction.update({
        ...
        data: { status: 'SUCCESSFUL', processorReference: verification.processorReference, paidAt: new Date() },
      });
    ...
    return { status: 'success', transaction: result.updated, payments: result.records };
  }

  // ─── Webhook handler ───────────────────────────────────────────────────────

  async handleWebhook(processor: string, body: any, rawBody: Buffer, headers: Record<string, string>) {
    ...
    if (!reference || !status) { this.logger.warn(`Unrecognised payload for ${processor}`); return; }

    try {
      if (status === 'success') {
        await this.verifyPayment({ reference, processor: processor as PaymentProcessor });
      } else {
  ...
  async getReceipt(receiptNumber: string) {
    const r = await (this.prisma as any).paymentRecord.findUnique({
      where: { receiptNumber },
      include: {
        fee:     { select: { feeName: true, session: true, term: true, amount: true } },
        student: { select: { firstName: true, lastName: true, registrationNumber: true } },
        transaction: {
          select: {
            reference: true, processor: true, channel: true,
            amountKobo: true, principalKobo: true, gatewayFeeKobo: true,
            platformTotalKobo: true, platformFlatKobo: true, platformPctKobo: true,
            paidAt: true,
          },
        },
      },
  ...
  async getStudentHistory(studentId: string, page = 1, limit = 20) {
    ...
    const [records, total] = await Promise.all([
      (this.prisma as any).paymentRecord.findMany({
        ...
        include: {
          ...
          transaction: { select: { reference: true, processor: true, channel: true, status: true, paidAt: true } },
        },
    ...
    return {
      records: records.map(r => ({ ...r, amountNaira: fromKobo(r.amountKobo) })),
  ...
  async getStudentOutstanding(studentId: string) {
    const student = await (this.prisma as any).student.findUnique({
      where: { id: studentId }, select: { schoolId: true },
    });
    if (!student) throw new NotFoundException('Student not found');

    const fees = await (this.prisma as any).feeConfiguration.findMany({ where: { schoolId: student.schoolId } });
    const result: any[] = [];

    for (const fee of fees) {
      const payments   = await (this.prisma as any).paymentRecord.findMany({
        where: { feeId: fee.id, studentId },
        select: { amountKobo: true, installmentNumber: true, paymentPlan: true },
      });
      const fullKobo   = toKobo(Number(fee.amount));
      const paidKobo   = payments.reduce((s, p) => s + p.amountKobo, 0);
      const paidCount  = payments.filter(p => p.paymentPlan === 'installments').length;

      result.push({
        feeId: fee.id, feeName: fee.feeName,
  ...
  private async getProcessorPaymentUrl(
    transaction: any, email: string, channel: PaymentChannel, split: SplitConfig,
  ): Promise<ProcessorResponse> {
    const meta = transaction.metadata as any;

    switch (transaction.processor) {
      case PaymentProcessor.PAYSTACK:
        return this.initiatePaystack(transaction.reference, transaction.amountKobo, email, channel, split, meta);
      case PaymentProcessor.FLUTTERWAVE:
        return this.initiateFlutterwave(transaction.reference, transaction.amountKobo, email, channel, split, meta);
      case PaymentProcessor.KORAPAY:
        return this.initiateKorapay(transaction.reference, transaction.amountKobo, email, channel, meta);
      default:
        throw new BadRequestException(`Unsupported processor: ${transaction.processor}`);
    }
  }
  ...
  private async initiatePaystack(
    reference: string, amountKobo: number, email: string,
    channel: PaymentChannel, split: SplitConfig, metadata?: any,
  ): Promise<ProcessorResponse> {
    const key = this.config.get<string>('PAYSTACK_SECRET_KEY');

    const payload: any = {
      reference, amount: amountKobo, email,
      channels: channel === PaymentChannel.BANK_TRANSFER
        ? ['bank_transfer']
        : ['card', 'bank', 'ussd', 'mobile_money', 'bank_transfer'],
      metadata: {
        custom_fields: [{ display_name: 'Student', variable_name: 'student', value: metadata?.studentName }],
      },
    };
    ...
    if (!res.data.status) throw new InternalServerErrorException('Paystack init failed');

    const data = res.data.data;
    return {
      paymentType: channel === PaymentChannel.BANK_TRANSFER ? 'bank_transfer' : 'card',
      paymentUrl: data.authorization_url,
      accessCode: data.access_code,
      ...(channel === PaymentChannel.BANK_TRANSFER && {
        virtualAccount: {
          bankName: data.bank ?? 'Wema Bank', accountNumber: data.account_number,
          accountName: data.account_name ?? 'School Fees', amount: amountKobo / 100,
  ...
  private async initiateFlutterwave(
    ...
    if (res.data.status !== 'success') throw new InternalServerErrorException('Flutterwave init failed');
  ...
  private async initiateKorapay(
    reference: string, amountKobo: number, email: string,
    channel: PaymentChannel, metadata?: any,
  ): Promise<ProcessorResponse> {
    const key = this.config.get<string>('KORAPAY_SECRET_KEY');

    const payload: any = {
      reference, amount: amountKobo / 100, currency: 'NGN',
      customer: { email, name: metadata?.studentName },
    ...
    if (channel === PaymentChannel.BANK_TRANSFER) {
      const res = await axios.post(
        'https://api.korapay.com/merchant/api/v1/virtual-bank-account', payload,
        { headers: { Authorization: `Bearer ${key}` } },
      );
      const d = res.data.data;
      return {
        paymentType: 'bank_transfer', paymentUrl: null,
        virtualAccount: { bankName: d.bank_name, accountNumber: d.account_number, accountName: d.account_name, amount: amountKobo / 100 },
      };
    }

    const res = await axios.post(
      'https://api.korapay.com/merchant/api/v1/charges/initialize', payload,
      { headers: { Authorization: `Bearer ${key}` } },
    );
  ...
  private async verifyWithProcessor(reference: string, processor: PaymentProcessor) {
    try {
      if (processor === PaymentProcessor.PAYSTACK) {
        const key = this.config.get<string>('PAYSTACK_SECRET_KEY');
        const res = await axios.get(`https://api.paystack.co/transaction/verify/${reference}`, { headers: { Authorization: `Bearer ${key}` } });
        const d = res.data.data;
        return { status: d.status === 'success' ? 'success' as const : 'failed' as const, processorReference: d.id?.toString() ?? reference };
      }
      if (processor === PaymentProcessor.FLUTTERWAVE) {
        const key = this.config.get<string>('FLUTTERWAVE_SECRET_KEY');
        const res = await axios.get(`https://api.flutterwave.com/v3/transactions?tx_ref=${reference}`, { headers: { Authorization: `Bearer ${key}` } });
        const tx = res.data.data?.[0];
        return { status: tx?.status === 'successful' ? 'success' as const : 'failed' as const, processorReference: tx?.id?.toString() ?? reference };
      }
      if (processor === PaymentProcessor.KORAPAY) {
        const key = this.config.get<string>('KORAPAY_SECRET_KEY');
        const res = await axios.get(`https://api.korapay.com/merchant/api/v1/charges/${reference}`, { headers: { Authorization: `Bearer ${key}` } });
        const d = res.data.data;
        return { status: d.status === 'success' ? 'success' as const : 'failed' as const, processorReference: d.payment_reference ?? reference };
      }
    } catch (e) {
  ...
  private verifyWebhookSignature(processor: string, raw: Buffer, headers: Record<string, string>): boolean {
    try {
      if (processor === PaymentProcessor.PAYSTACK)
        return verifyPaystackSig(raw, headers['x-paystack-signature'], this.config.get('PAYSTACK_SECRET_KEY')!);
      if (processor === PaymentProcessor.FLUTTERWAVE)
        return headers['verif-hash'] === this.config.get('FLUTTERWAVE_WEBHOOK_HASH');
      if (processor === PaymentProcessor.KORAPAY)
        return verifyKorapaySig(raw, headers['x-korapay-signature'], this.config.get('KORAPAY_ENCRYPTION_KEY')!);
    } catch { return false; }
    return false;
  }
